...
class SessionViewSetTests(TestCase):
    ...
    def test_create_session(self):
        ...
        self.assertTrue(SurveySession.objects.filter(pk=session_id, code=session_code).exists())
    ...
    def test_list_session(self):
        ...
        session1 = SurveySession(survey=self.survey, code=1234, owner=self.user)
        session1.save()
    ...
    def test_fetch_question(self):
        ...
        # create a question
        instance = SurveySession(survey=self.survey, code=1234, owner=self.user)
        instance.save()
...
class DuplicateSurveyViewSetTests(TestCase):
    ...
    def test_duplicate_survey(self):
        ...
        self.assertTrue(SurveyQuestion.objects.filter(survey=self.survey.id).exists())

        # get the original and duplicated question
        question1 = list(SurveyQuestion.objects.filter(survey=self.survey.id).values())[0]
        question2 = list(SurveyQuestion.objects.filter(survey=survey_id).values())[0]
        question1.pop('id') 
        ...
        new_choice = SurveyQuestionChoice.objects.filter(question=new_question_id)
        self.assertTrue(new_choice.exists())
        new_choice_id = list(new_choice)[0].id
        self.assertNotEqual(new_choice_id, self.choice.id)
        # check that the old question wasn't overriden
        self.assertTrue(SurveyQuestion.objects.filter(survey=self.choice.id).exists())

        # get the original and duplicated choice
        choice1 = list(SurveyQuestionChoice.objects.filter(question=self.choice.id).values())[0]
        choice2 = list(SurveyQuestionChoice.objects.filter(question=new_choice_id).values())[0]
        choice1.pop('id') 
